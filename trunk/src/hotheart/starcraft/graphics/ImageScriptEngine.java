...
public class ImageScriptEngine {
	...
	public static final ImageScriptHeader createHeader(int id)
	{
		...
		while(true)
		{
			int num = (script[offset++]&0xFF) + ((script[offset++]&0xFF)<<8);
			int dstOffset = (script[offset++]&0xFF) + ((script[offset++]&0xFF)<<8);
	...
	public static final void exec(Image instance)
	{
		...
		switch(opcode)
		{
			...
			case OP_PLAY_SOUND://Play sound
				int sound_id =  (script[instance.scriptPos ++ ]&0xFF) + ((script[instance.scriptPos ++ ]&0xFF)<<8);
				StarcraftSoundPool.playSound(sound_id);
				break;
			case OP_PLAY_SOUND_BETWEEN:
				int sound_start =  (script[instance.scriptPos ++ ]&0xFF) + ((script[instance.scriptPos ++ ]&0xFF)<<8);
				int sound_end =  (script[instance.scriptPos ++ ]&0xFF) + ((script[instance.scriptPos ++ ]&0xFF)<<8);
				
				StarcraftSoundPool.playSound(
						sound_start + rnd.nextInt(sound_end - sound_start));
				break;
			...
			case 0x27:
			default:
				Log.e("UNIMPLEMENTED","Unimplemented opcode: 0x" + Integer.toHexString(opcode));
				break;
